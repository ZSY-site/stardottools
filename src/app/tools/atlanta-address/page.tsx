...
// 亚特兰大地址生成器详情页面
export default function AtlantaAddressGeneratorPage() {
  ...
  const atlantaNeighborhoods = [
    { name: 'Midtown', zipCode: '30308' },
    { name: 'Buckhead', zipCode: '30305' },
    { name: 'Downtown', zipCode: '30303' },
    { name: 'Virginia-Highland', zipCode: '30306' },
    { name: 'Inman Park', zipCode: '30307' },
    { name: 'Old Fourth Ward', zipCode: '30308' },
    { name: 'Grant Park', zipCode: '30312' },
    { name: 'West End', zipCode: '30310' },
    { name: 'East Atlanta', zipCode: '30316' },
    { name: 'Decatur', zipCode: '30030' }
  ]
  ...
  // 亚特兰大街道名称
  const atlantaStreets = [
    'Peachtree Street', 'Piedmont Avenue', 'North Avenue', 'Spring Street',
    'Marietta Street', 'Decatur Street', 'Edgewood Avenue', 'Ponce de Leon Avenue',
    'Monroe Drive', 'Briarcliff Road', 'Cheshire Bridge Road', 'Buford Highway'
  ]

  // 生成随机美国手机号
  const generateUSPhone = () => {
    const areaCode = '404' // 亚特兰大区号
  ...
  // 生成随机亚特兰大地址
  const generateAtlantaAddress = () => {
    const neighborhood = atlantaNeighborhoods[Math.floor(Math.random() * atlantaNeighborhoods.length)]
    const street = atlantaStreets[Math.floor(Math.random() * atlantaStreets.length)]
    const building = Math.floor(Math.random() * 5000) + 1
    const apartment = Math.floor(Math.random() * 500) + 1
  ...
  // 生成亚特兰大身份数据
  const generateAtlantaIdentity = () => {
    const gender = genderOption === 'random' 
      ? (Math.random() > 0.5 ? 'male' : 'female')
      : genderOption
    
    const names = americanNames[gender]
    ...
    const address = generateAtlantaAddress()
    ...
    return {
      ...
      fullAddress: `${address.street}, ${address.apartment}, ${address.city}, ${address.state} ${address.zipCode}, ${address.country}`
    }
  }
  ...
  const generateData = () => {
    ...
    for (let i = 0; i < generationCount; i++) {
      data.push(generateAtlantaIdentity())
    }
  ...
  const exportData = () => {
    if (exportFormat === 'json') {
      ...
      link.href = url
      link.download = 'atlanta_addresses.json'
      link.click()
    } else {
      // CSV导出
      ...
      const link = document.createElement('a')
      link.href = url
      link.download = 'atlanta_addresses.csv'
      link.click()
    }
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-lg text-gray-600 max-w-3xl mx-auto">
            快速生成符合亚特兰大本地格式的真实地址数据，包含完整街道、社区、邮编信息，同时支持生成配套的虚拟身份数据。
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          <div className="bg-white rounded-xl shadow-lg p-6 text-center">
            ...
            <h3 className="font-semibold text-gray-900 mb-2">亚特兰大本地化</h3>
            <p className="text-gray-600 text-sm">生成符合亚特兰大格式的身份信息，包括姓名、地址、手机号等</p>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-8 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="text-center p-6">
              ...
              <p className="text-gray-600 text-sm">在开发和测试阶段，快速生成大量符合亚特兰大格式的身份数据</p>
            </div>
            ...
            <div className="text-center p-6">
              ...
              <p className="text-gray-600 text-sm">为需要支持美国本地化的应用提供符合当地格式的测试数据</p>
            </div>
            ...
            <div className="text-center p-6">
              ...
              <p className="text-gray-600 text-sm">在教学或演示需要美国身份数据的场景时，提供安全、虚构的数据样本</p>
            </div>
            ...
            <div className="text-center p-6">
              ...
              <p className="text-gray-600 text-sm">在需要美国身份信息的场景中，使用随机生成的数据，避免泄露真实个人信息</p>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-gray-600 text-sm">
                不是，所有生成的数据都是完全虚拟的，仅用于测试和演示目的。这些数据符合亚特兰大本地格式，但都是随机生成的，与真实个人无关。
              </p>
            </div>
            ...
            <div>
              ...
              <p className="text-gray-600 text-sm">
                适用于：表单测试、数据库填充、API测试、UI设计演示、教育培训、软件开发等场景。特别适合需要美国本地化数据的开发者。
              </p>
            </div>
            ...
            <div>
              ...
              <p className="text-gray-600 text-sm">
                是的，地址格式严格按照美国标准：街道地址 + 城市 + 州 + 邮政编码 + 美国。包含亚特兰大主要社区的真实数据。
              </p>
            </div>
          </div>
        </div>
